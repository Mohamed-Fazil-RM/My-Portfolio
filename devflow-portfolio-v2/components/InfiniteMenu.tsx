...
void main() {
    ...
    vAlpha = smoothstep(0.4, 1., normalize(worldPosition.xyz).z) * .95 + .05;
    vUvs = aModelUvs;
...
class ArcballControl {
  ...
  constructor(public canvas: HTMLCanvasElement, public updateCallback: (dt: number) => void) {
...
class InfiniteGridMenu {
  ...
  constructor(public canvas: HTMLCanvasElement, public items: any[], public onActiveItemChange: (idx: number) => void, public onMovementChange: (m: boolean) => void, onInit: (sk: InfiniteGridMenu) => void, public scaleFactor = 1.0) {
  ...
  resize() {
    ...
    this.viewportSize = vec2.set(this.viewportSize || vec2.create(), canvas.clientWidth, canvas.clientHeight);
    if (resizeCanvasToDisplaySize(canvas)) this.gl.viewport(0, 0, this.gl.drawingBufferWidth, this.gl.drawingBufferHeight);
  ...
  #init(onInit: (sk: InfiniteGridMenu) => void) {
    const gl = this.canvas.getContext('webgl2', { antialias: true, alpha: true });
    if (!gl) throw new Error('No WebGL 2 context!');
    this.gl = gl;
    this.discProgram = createProgram(gl, [discVertShaderSource, discFragShaderSource], undefined, { aModelPosition: 0, aModelNormal: 1, aModelUvs: 2, aInstanceMatrix: 3 })!;
    this.discLocations = {
  ...
  #animate(deltaTime: number) {
    ...
    this.instancePositions.forEach((p, ndx) => {
      const pt = vec3.transformQuat(vec3.create(), p, this.control.orientation);
      const s = (Math.abs(pt[2]) / this.SPHERE_RADIUS) * SCALE_INTENSITY + (1 - SCALE_INTENSITY);
      const finalScale = s * scale;
      const matrix = mat4.create();
      mat4.multiply(matrix, matrix, mat4.fromTranslation(mat4.create(), vec3.negate(vec3.create(), pt)));
      mat4.multiply(matrix, matrix, mat4.targetTo(mat4.create(), [0, 0, 0], pt, [0, 1, 0]));
  ...
  #updateProjectionMatrix(gl: WebGL2RenderingContext) {
    ...
    this.camera.fov = 2 * Math.atan(this.camera.aspect > 1 ? height / distance : height / this.camera.aspect / distance);
  ...
  #onControlUpdate(deltaTime: number) {
    ...
    } else { cameraTargetZ += this.control.rotationVelocity * 80 + 2.5; damping = 7 / timeScale; }
  ...
  #findNearestVertexIndex() {
    ...
    const nt = vec3.transformQuat(vec3.create(), n, quat.conjugate(quat.create(), this.control.orientation));
    let maxD = -1, idx = 0;
...
export default function InfiniteMenu({ items = [], scale = 1.0 }: { items: any[], scale?: number }) {
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    const sketch = new InfiniteGridMenu(canvas, items.length ? items : [{ image: 'https://picsum.photos/900/900?grayscale', title: '', description: '' }], idx => setActiveItem(items[idx % items.length]), setIsMoving, sk => sk.run(), scale);
  ...
  const handleButtonClick = () => { if (activeItem?.link) window.open(activeItem.link, '_blank'); };
  ...
  return (
    <div className="relative w-full h-full overflow-hidden bg-white dark:bg-black transition-colors duration-500">
      ...
      {/* Static Section Title - Top Middle Fixed */}
      <div className="section-title">Curated Works</div>
      ...
      {activeItem && (
        <>
          ...
          {/* Action button anchored correctly below focused circle */}
