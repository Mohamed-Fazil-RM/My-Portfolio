...
function SplashCursor({
  ...
  useEffect(() => {
    ...
    const handlePointerDown = (clientX: number, clientY: number, identifier: number = -1) => {
      const rect = canvas.getBoundingClientRect();
      if (clientX < rect.left || clientX > rect.right || clientY < rect.top || clientY > rect.bottom) return;
      
      const pointer = pointers[0];
      let posX = scaleByPixelRatio(clientX - rect.left);
      let posY = scaleByPixelRatio(clientY - rect.top);
      updatePointerDownData(pointer, identifier, posX, posY);
      clickSplat(pointer);
    };
    ...
    const handlePointerMove = (clientX: number, clientY: number) => {
      ...
      // Even if mouse is outside, we process the update to allow fluid dissipation
      let posX = scaleByPixelRatio(clientX - rect.left);
      let posY = scaleByPixelRatio(clientY - rect.top);
      
      // Only splat if mouse is actually over the box
    ...
    const onTouchStart = (e: TouchEvent) => {
      if (e.targetTouches[0]) handlePointerDown(e.targetTouches[0].clientX, e.targetTouches[0].clientY, e.targetTouches[0].identifier);
    };
    const onTouchMove = (e: TouchEvent) => {
      if (e.targetTouches[0]) handlePointerMove(e.targetTouches[0].clientX, e.targetTouches[0].clientY);
    };
    const onMouseUp = () => updatePointerUpData(pointers[0]);

    // Attach to window so we can track movement even if masking elements are present, 
    // but the coordinates are scoped to the canvas bounds inside the functions.
